 ...
 *    Limitation: a thread that unblocks a higher-priority thread under the lock
 *    keeps running until that next tick. The full fix needs the global-lock
 *    release path to honour the flag (or a real deferred-switch interrupt).
 */
