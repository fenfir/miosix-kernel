 ...
 *   and does not perturb it. 42 MHz, measured on silicon.                  *
 *                                                                          *
...
#include "interfaces/arch_registers.h"   // HRC7000_T1_CURVAL, HRC7000_TIMER_HZ

namespace miosix {

void delayUs(unsigned int useconds)
{
    const unsigned int countsPerUs=HRC7000_TIMER_HZ/1000000u;   //42
    const unsigned int target=useconds*countsPerUs;
    unsigned int last=HRC7000_T1_CURVAL;                         //ch0 down-counter
    unsigned int elapsed=0;
    while(elapsed<target)
    {
        unsigned int cur=HRC7000_T1_CURVAL;
